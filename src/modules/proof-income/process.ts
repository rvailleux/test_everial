/**
 * Processing Logic for Proof of Income Module
 *
 * Handles document processing via WIZIDEE APIs.
 * Implements the standard two-phase workflow:
 *   1. Recognition - identify document type (dbtype: income_proof)
 *   2. Analysis - extract income data fields
 */

import { WizideeResult } from '@/lib/modules/types';
import { IncomeConfig, IncomeData } from './types';
...
/**
 * Process a document snapshot for income extraction
 *
...
export async function process(
  ...
  try {
    ...
    const file = new File([snapshot], 'income-document.png', { type: 'image/png' });
    ...
    // ============================================================================
    // STEP 3: Extract Income Data
    // ============================================================================
    const incomeData = extractIncomeData(analysis);
...
/**
 * Extract income-specific fields from WIZIDEE analyze response
 *
 * Maps various possible field names from WIZIDEE to our standard IncomeData structure.
 *
 * @param analyzeResult - Raw analyze response from WIZIDEE
 * @returns Structured income data
 */
function extractIncomeData(analyzeResult: AnalyzeResponse): IncomeData {
  const fields = analyzeResult.fields || {};
  const confidence = analyzeResult.confidence;

  return {
    revenus: extractField(fields, ['revenus', 'revenue', 'income', 'salaire', 'salary', 'montant', 'amount', 'total']),
    employeur: extractField(fields, ['employeur', 'employer', 'entreprise', 'company', 'societe', 'organisme', 'taxAuthority']),
    periode: extractField(fields, ['periode', 'period', 'periodePaie', 'payPeriod', 'mois', 'month', 'annee', 'year']),
    netPay: extractField(fields, ['netPay', 'net', 'salaireNet', 'netSalary', 'aPayer', 'toPay']),
    grossPay: extractField(fields, ['grossPay', 'brut', 'salaireBrut', 'grossSalary', 'totalBrut']),
    employeeName: extractField(fields, ['employeeName', 'nomEmploye', 'employee', 'salarie', 'contribuable']),
    dateDocument: extractField(fields, ['dateDocument', 'documentDate', 'date', 'dateEmission', 'datePaiement']),
    taxYear: extractField(fields, ['taxYear', 'anneeFiscale', 'fiscalYear', 'anneeImpots']),
